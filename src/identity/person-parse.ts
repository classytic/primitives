/**
 * `PersonName` PARSING and validation — the inverse of `person.ts`'s formatting.
 *
 * `person.ts` turns a structured `PersonName` into a string (`formatFullName`,
 * `formatDisplayName`). This file turns the messy string that arrives at an auth / POS /
 * checkout boundary back INTO a structured `PersonName`, and decides whether what arrived
 * is a name at all.
 *
 * ## Why this is a primitive and not commerce code
 *
 * It lived in be-prod as `customer.name-utils.ts`, but nothing in it is about customers —
 * six unrelated subsystems already imported it (party linking, invoicing, logistics
 * exports, transaction statements). Any vertical with PEOPLE has the same problem: a gym
 * member, a hotel guest and a clinic patient all arrive as a flat string from an identity
 * provider, and all can arrive as an opaque provider id instead of a name.
 *
 * Pure and side-effect free — no I/O, no config, no locale service.
 */
...
export function fallbackNameFromUser(
  ...
  if (email?.includes('@')) {
    ...
    // Unreachable in practice (we already checked for '@'), but an empty local-part then
    // falls through to `defaultLabel` rather than throwing on `.replace`.
