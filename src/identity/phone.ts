...
const E164_PREFIXES_3: ReadonlySet<string> = new Set([
  ...
  '692', '800', '808', '850', '852', '853', '855', '856', '870', '878',
  '880', '881', '882', '883', '886', '888',
  '960', '961', '962', '963', '964', '965', '966', '967', '968', '970',
  '971', '972', '973', '974', '975', '976', '977', '979', '992', '993',
  '994', '995', '996', '998',
]);
...
export function parsePhone(input: string): Result<PhoneNumber, PhoneError> {
  ...
  if (digits.length > 15) {
    return err(
      new PhoneError(
        'TOO_LONG',
        `phone exceeds E.164 15-digit limit: ${digits.length} digits`,
      ),
    );
  }
  if (digits.length < 7) {
    return err(
      new PhoneError(
        'TOO_SHORT',
        `phone too short to be E.164: ${digits.length} digits`,
      ),
    );
  }
  ...
  const callingCode =
    E164_PREFIXES_3.has(digits.slice(0, 3))
      ? digits.slice(0, 3)
  ...
  if (callingCode === null) {
    return err(
      new PhoneError(
        'NOT_E164',
        `unrecognized country calling code in ${JSON.stringify(input)}`,
      ),
    );
  }
