...
describe('evaluate — field conditions', () => {
  ...
  it('in / nin against arrays', () => {
    expect(evaluate({ field: 'priority', op: 'in', value: ['high', 'urgent'] }, target)).toBe(true);
    expect(evaluate({ field: 'priority', op: 'nin', value: ['low', 'medium'] }, target)).toBe(true);
  });
  ...
  it('handles Date comparisons (Date vs Date and Date vs ISO string)', () => {
    expect(
      evaluate({ field: 'closedAt', op: 'gt', value: new Date('2026-01-01T00:00:00Z') }, target),
    ).toBe(true);
    expect(
...
describe('evaluate — composite conditions', () => {
  ...
  it('not inverts the wrapped condition', () => {
    expect(evaluate({ not: { field: 'status', op: 'eq', value: 'lost' } }, target)).toBe(true);
  });
...
describe('validateCondition', () => {
  ...
  it('rejects unknown comparator (INVALID_OP)', () => {
    expect(() => validateCondition({ field: 'x', op: 'matches' as any, value: 1 })).toThrow(
      ConditionError,
    );
  });
